...
data InstrumentationLibrary
  = InstrumentationLibrary'_constructor {_InstrumentationLibrary'name :: !Data.Text.Text,
...
instance Data.ProtoLens.Message InstrumentationLibrary where
  ...
  parseMessage
    = let
      ...
      in
        (Data.ProtoLens.Encoding.Bytes.<?>)
          (do loop Data.ProtoLens.defMessage) "InstrumentationLibrary"
  buildMessage
    = \ _x
        -> (Data.Monoid.<>)
...
data StringKeyValue
  = StringKeyValue'_constructor {_StringKeyValue'key :: !Data.Text.Text,
...
instance Data.ProtoLens.Field.HasField StringKeyValue "key" Data.Text.Text where
  fieldOf _
    = (Prelude..)
        (Lens.Family2.Unchecked.lens
           _StringKeyValue'key (\ x__ y__ -> x__ {_StringKeyValue'key = y__}))
        Prelude.id
...
instance Data.ProtoLens.Message StringKeyValue where
  ...
  parseMessage
    = let
      ...
      in
        (Data.ProtoLens.Encoding.Bytes.<?>)
          (do loop Data.ProtoLens.defMessage) "StringKeyValue"
  buildMessage
    = \ _x
        -> (Data.Monoid.<>)
             (let _v = Lens.Family2.view (Data.ProtoLens.Field.field @"key") _x
