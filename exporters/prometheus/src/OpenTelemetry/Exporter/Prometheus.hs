...
renderPrometheusText batches =
  TL.toStrict . toLazyText $
    mconcat (intersperse nl (fmap renderResource batches))
        ...
        <> V.foldl'
          ( \acc p ->
              acc
                ...
                <> sp
                <> either decimal buildDouble (sumDataPointValue p)
        ...
        <> V.foldl'
          ( \acc p ->
              acc
                ...
                <> sp
                <> either decimal buildDouble (gaugeDataPointValue p)
...
exemplarValue e = case metricExemplarValue e of
  Nothing -> "0"
  Just (Left i) -> decimal i
  Just (Right d) -> buildDouble d
...
primitiveToText = \case
  ...
  IntAttribute i -> TL.toStrict $ toLazyText $ decimal (i :: Int64)
