...
builderText :: LB.Builder -> Text
builderText = LT.toStrict . LB.toLazyText


textIntegral :: (Integral n) => n -> Text
textIntegral = builderText . decimal


textDouble :: Double -> Text
          ...
          fmap
            ( \p ->
                T.concat
                  ...
                  , " "
                  , either textIntegral textDouble (sumDataPointValue p)
          ...
          fmap
            ( \p ->
                T.concat
                  ...
                  , " "
                  , either textIntegral textDouble (gaugeDataPointValue p)
...
exemplarValueText e = case metricExemplarValue e of
  Nothing -> "0"
  Just (Left i) -> textIntegral i
  Just (Right d) -> textDouble d
  ...
  let lbls = mergeLabels baseLabels (attributesToLabelMap (histogramDataPointAttributes p))
        ...
        fmap
          ( \(b, c) ->
              T.concat
                [ hname
                , "_bucket"
                , formatLabels (Map.insert "le" (textDouble b) lbls)
                , " "
                , textIntegral c
                ]
          )
      ...
      infLine =
        T.concat
          ...
          , " "
          , textIntegral (histogramDataPointCount p)
      ...
      sumLine =
        T.concat
          ...
          , formatLabels lbls
          , " "
          , textDouble (histogramDataPointSum p)
          ]
      countLine =
        T.concat
          [ hname
  ...
  let lbls = mergeLabels baseLabels (attributesToLabelMap (exponentialHistogramDataPointAttributes p))
        ...
        fmap
          ( \(i, c) ->
              ...
              in T.concat
                   [ hname
                   , "_bucket"
                   , formatLabels (Map.insert "le" (textDouble le) lbls)
                   , " "
                   , textIntegral c
                   ]
          )
        ...
        if exponentialHistogramDataPointZeroCount p == 0
          then []
            ...
            [ T.concat
                ...
                , " "
                , textIntegral (exponentialHistogramDataPointZeroCount p)
                ]
            ]
      infLine =
        T.concat
          ...
          , formatLabels (Map.insert "le" "+Inf" lbls)
          , " "
          , textIntegral (exponentialHistogramDataPointCount p)
      ...
      sumLine =
        T.concat
          ...
          , formatLabels lbls
          , " "
          , textDouble (fromMaybe 0 (exponentialHistogramDataPointSum p))
          ]
      countLine =
        T.concat
          [ hname
...
primitiveToText = \case
  ...
  DoubleAttribute d -> textDouble d
  IntAttribute i -> textIntegral (i :: Int64)
