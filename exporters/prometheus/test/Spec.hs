...
main = hspec $ do
  ...
  describe "renderPrometheusText" $ do
    ...
    it "includes HELP and TYPE for a gauge" $ do
      let out = renderPrometheusText [wrap lib $ MetricExportGauge "cpu" "cpu usage" "1" lib True $ V.singleton (mkGaugePt (DoubleNumber 0.5))]
    ...
    it "renders monotonic sum as counter with correct value" $ do
      let out = renderPrometheusText [wrap lib $ MetricExportSum "http_requests" "total" "1" lib True False AggregationCumulative $ V.singleton (mkSumPt (DoubleNumber 42.0))]
    ...
    it "renders non-monotonic sum as gauge" $ do
      let out = renderPrometheusText [wrap lib $ MetricExportSum "queue" "depth" "1" lib False False AggregationDelta $ V.singleton (mkSumPt (IntNumber (-3)))]
    ...
    it "renders Int64 counter as integer string" $ do
      let out = renderPrometheusText [wrap lib $ MetricExportSum "ops" "" "1" lib True True AggregationCumulative $ V.singleton (mkSumPt (IntNumber 99))]
      out `shouldSatisfy` T.isInfixOf "ops{job=\"test-lib\"} 99\n"

    -- Name sanitization
    it "sanitizes dots to underscores in metric names" $ do
      let out = renderPrometheusText [wrap lib $ MetricExportGauge "http.server.duration" "" "ms" lib True $ V.singleton (mkGaugePt (DoubleNumber 1.0))]
    ...
    it "sanitizes hyphens and slashes in metric names" $ do
      let out = renderPrometheusText [wrap lib $ MetricExportGauge "my-metric/rate" "" "1" lib True $ V.singleton (mkGaugePt (DoubleNumber 0.0))]
    ...
    it "sanitizes leading digit in label name to underscore prefix" $ do
      let attrs = addAttribute defaultAttributeLimits emptyAttributes "0bad" ("val" :: T.Text)
          ...
          out = renderPrometheusText [wrap lib $ MetricExportGauge "m" "" "1" lib True $ V.singleton pt]
    ...
    it "escapes backslash in HELP text" $ do
      let out = renderPrometheusText [wrap lib $ MetricExportGauge "m" "path is C:\\foo" "1" lib True $ V.singleton (mkGaugePt (DoubleNumber 1.0))]
      out `shouldSatisfy` T.isInfixOf "# HELP m path is C:\\\\foo\n"

    it "escapes newline in HELP text" $ do
      let out = renderPrometheusText [wrap lib $ MetricExportGauge "m" "line1\nline2" "1" lib True $ V.singleton (mkGaugePt (DoubleNumber 1.0))]
      out `shouldSatisfy` T.isInfixOf "# HELP m line1\\nline2\n"

    -- Special float values
    it "renders +Inf gauge value as +Inf" $ do
      let out = renderPrometheusText [wrap lib $ MetricExportGauge "m" "" "1" lib True $ V.singleton (mkGaugePt (DoubleNumber (1 / 0)))]
    ...
    it "renders NaN gauge value as NaN" $ do
      let out = renderPrometheusText [wrap lib $ MetricExportGauge "m" "" "1" lib True $ V.singleton (mkGaugePt (DoubleNumber (0 / 0)))]
    ...
    it "renders histogram with correct cumulative bucket counts" $ do
      let pt =
          ...
          out = renderPrometheusText [wrap lib $ MetricExportHistogram "lat" "latency" "ms" lib AggregationCumulative $ V.singleton pt]
    ...
    it "histogram renders all line families in correct order" $ do
      let pt =
          ...
          out = renderPrometheusText [wrap lib $ MetricExportHistogram "h" "" "1" lib AggregationCumulative $ V.singleton pt]
          ls = T.lines out
    ...
    it "renders exponential histogram with positive buckets" $ do
      let pt =
          ...
          out = renderPrometheusText [wrap lib $ MetricExportExponentialHistogram "eh" "" "1" lib AggregationCumulative $ V.singleton pt]
    ...
    it "resource attributes appear as labels on metric lines" $ do
      let res = materializeResourcesWithSchema Nothing (mkResource ["cloud.region" .= ("us-east-1" :: T.Text)])
    ...
    it "renders multiple data points per metric" $ do
      let a1 = addAttribute defaultAttributeLimits emptyAttributes "route" ("/a" :: T.Text)
          ...
          out = renderPrometheusText [wrap lib $ MetricExportGauge "rps" "" "1" lib True $ V.fromList [pt1, pt2]]
    ...
    it "omits job label when scope name is empty" $ do
      let emptyLib = InstrumentationLibrary "" "" "" emptyAttributes
          out = renderPrometheusText [wrap emptyLib $ MetricExportGauge "m" "" "1" emptyLib True $ V.singleton (mkGaugePt (DoubleNumber 1.0))]
    ...
    it "renders exemplar comment on counter line" $ do
      let ex =
          ...
          out = renderPrometheusText [wrap lib $ MetricExportSum "c" "" "1" lib True True AggregationCumulative $ V.singleton pt]
    ...
    it "Int64 gauge renders without decimal" $ do
      let out = renderPrometheusText [wrap lib $ MetricExportGauge "m" "" "1" lib True $ V.singleton (mkGaugePt (IntNumber 42))]
      out `shouldSatisfy` T.isInfixOf "m{job=\"test-lib\"} 42\n"

    it "large Int64 renders without scientific notation" $ do
      let out = renderPrometheusText [wrap lib $ MetricExportSum "big" "" "1" lib True True AggregationCumulative $ V.singleton (mkSumPt (IntNumber 9999999999))]
      out `shouldSatisfy` T.isInfixOf "big{job=\"test-lib\"} 9999999999\n"

    it "negative gauge renders correctly" $ do
      let out = renderPrometheusText [wrap lib $ MetricExportGauge "m" "" "1" lib True $ V.singleton (mkGaugePt (IntNumber (-7)))]
